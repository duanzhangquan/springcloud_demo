...
/**
 * jwt token校验过滤器,
 * 对不支持匿名访问的接口进行统一拦截,如果未携带token或token不合法或已过期，则身份认证失败.
 * 否则对要访问的业务接口放行
 */
@Slf4j
@Component
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(url.lastIndexOf("/ldap/login") >-1){
            log.debug("登录接口不做拦截");
        ...
        UserTokenVO userTokenVO = new UserTokenVO(token,refreshToken,userAccount, AccountTypeEnum.LDAP_USER.getValue());
        boolean validated = customAuthenticationService.validateUser(userTokenVO,response);
        customAuthenticationService.setLdapPersonService(ldapPersonService);
        if(!validated){
